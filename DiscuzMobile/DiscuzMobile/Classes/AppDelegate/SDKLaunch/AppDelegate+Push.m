...
//        [[DZPushCenter shareInstance] getNotiToview:pushNotificationKey];
//        //这里定义自己的处理方式
//        [[DZPushCenter shareInstance] receiveNotificationFeeback:pushNotificationKey];
//    } else {
//        [[DZPushCenter shareInstance] cancelPushBadge];
//    }
}
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions))completionHandler {
    [[DZPushCenter shareInstance] isActivePushAlert:notification.request.content.userInfo];
}

// (APP在后台)
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler {
    
    [[DZPushCenter shareInstance] getNotiToview:response.notification.request.content.userInfo];
}
...
- (void)application:(UIApplication*)application didReceiveRemoteNotification:(NSDictionary*)userInfo
{
    if([UIApplication sharedApplication].applicationState == UIApplicationStateActive) {
        // 本来就在前台的时候
        [[DZPushCenter shareInstance] isActivePushAlert:userInfo];
        
    } else if ([UIApplication sharedApplication].applicationState == UIApplicationStateInactive) {
        //        点击推送通知进入界面的时候
        [[DZPushCenter shareInstance] getNotiToview:userInfo];
    }
    
    [[DZPushCenter shareInstance] receiveNotificationFeeback:userInfo];
}
