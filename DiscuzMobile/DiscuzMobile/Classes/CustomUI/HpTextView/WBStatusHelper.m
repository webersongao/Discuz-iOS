...
+ (NSBundle *)bundle {
    ...
    dispatch_once(&onceToken, ^{
        NSString *path = [[NSBundle mainBundle] pathForResource:@"ResourceJT" ofType:@"bundle"];
...
+ (NSBundle *)emoticonBundle {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"EmoticonJT" ofType:@"bundle"];
...
+ (NSRegularExpression *)regexEmoticon {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // (\\[[^ \\[\\]]+?\\])|(\\#\\[[0-9]{2,3}\\_[0-9]{2,3}\\])       \\[[^ \\[\\]]+?\\] = [微笑]   \\#\\[[0-9]{2,3}\\_[0-9]{2,3}\\] = #[01_01]
        regex = [NSRegularExpression regularExpressionWithPattern:@"\\#\\[[0-9]{2,3}\\_[0-9]{2,3}\\]" options:kNilOptions error:NULL];
    });
    return regex;
}
...
+ (NSDictionary *)emoticonDic {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *emoticonBundlePath = [[NSBundle mainBundle] pathForResource:@"EmoticonJT" ofType:@"bundle"];
...
+ (NSMutableDictionary *)_emoticonDicFromPath:(NSString *)path {
    ...
    if (!group) {
        NSString *plistPath = [path stringByAppendingPathComponent:@"Eminfo.plist"];
...
+ (NSArray<WBEmoticonGroup *> *)emoticonGroups {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            ...
            NSString *infoPlistPath = [path stringByAppendingPathComponent:@"Eminfo.plist"];
