...
@property(nonatomic,strong) NSMutableArray<DZForumOldListController*> *listViewArray;
...
-(void)setUpSegmentHeaderView:(DZVIPCategoryInfoModel *)dataModel{
    ...
    if (dataModel.category.count) {
        ...
        for (int i = 0; i < categoryCount; i++)
        {
            ...
            DZForumOldListController *listVC = [[DZForumOldListController alloc] initWithFrame:CGRectMake(i * self.scrollView.width, 0, self.scrollView.width, self.scrollView.height)];
...
//        self.sepLineView.frame = CGRectMake(0, self.topBar.bottom-0.5, KScreenWidth, 0.5);
        ...
        DZForumOldListController *listVC = [[DZForumOldListController alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, self.scrollView.height)];
        listVC.subtype = @"0";
        self.currentIndex = 0;
        listVC.column = self.columntype;
        [self.scrollView addSubview:listVC.view];
