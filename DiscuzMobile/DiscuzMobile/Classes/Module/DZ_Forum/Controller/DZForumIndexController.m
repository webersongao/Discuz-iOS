...
@interface DZForumIndexController ()<UIScrollViewDelegate,PRNaviSegmentViewDelegate>
...
@implementation DZForumIndexController
...
-(void)layoutForumList:(NSArray <DZCateListModel *>*)cateList forumList:(NSArray <DZBaseForumModel *>*)forumList{
    ...
    for (int index = 0; index < forumList.count; index ++) {
        ...
        DZForumCateListController *listVC = [[DZForumCateListController alloc] initWithFrame:CGRectMake(index * self.scrollView.width, 0, self.scrollView.width, self.scrollView.height) Model:model];
