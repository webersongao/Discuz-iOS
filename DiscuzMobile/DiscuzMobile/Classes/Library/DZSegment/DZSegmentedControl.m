...
@property (nonatomic, strong) CALayer *badageLab;
...
@implementation DZSegmentedControl
...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.type = DZSegmentedControlTypeText;
    }
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = DZSegmentedControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
        self.type = DZSegmentedControlTypeTextImages;
    }
...
- (void)commonInit {
    ...
    self.selectionStyle = DZSegmentedControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = DZSegmentedControlSelectionIndicatorLocationUp;
    self.segmentWidthStyle = DZSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(DZSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == DZSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(DZSegmentedControlSegmentWidthStyle)segmentWidthStyle {
    // Force DZSegmentedControlSegmentWidthStyleFixed when type is DZSegmentedControlTypeImages.
    if (self.type == DZSegmentedControlTypeImages) {
        _segmentWidthStyle = DZSegmentedControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(DZSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == DZSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == DZSegmentedControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != DZSegmentedControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == DZSegmentedControlSegmentWidthStyleFixed) {
            ...
            } else if (self.segmentWidthStyle == DZSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == DZSegmentedControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == DZSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
    ...
    } else if (self.type == DZSegmentedControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != DZSegmentedControlNoSegment) {
        if (self.selectionStyle == DZSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == DZSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)setBadge:(CGRect)frame {
    self.badageLab.frame = [self frameForSelectionIndicator];
    [self.scrollView.layer insertSublayer:self.badageLab atIndex:0];
}

- (CALayer *)badageLab {
    if (!_badageLab) {
        _badageLab = [CALayer layer];
        _badageLab.backgroundColor = [UIColor redColor].CGColor;
        _badageLab.contentsScale = [[UIScreen mainScreen] scale];
    }
    return _badageLab;
}
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & DZSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == DZSegmentedControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == DZSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == DZSegmentedControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == DZSegmentedControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != DZSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
        ...
        else if (self.selectionStyle == DZSegmentedControlSelectionStyleTextWidthMorelittle &&
                 sectionWidth <= self.segmentWidth &&
                 self.segmentWidthStyle != DZSegmentedControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            
            CGFloat x = ((widthToEndOfSelectedSegment - widthToStartOfSelectedIndex) / 2) + (widthToStartOfSelectedIndex - (sectionWidth + 14) / 2);
            return CGRectMake(x + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, (sectionWidth + 14) - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight);
        }
        
        else {
            if (self.segmentWidthStyle == DZSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == DZSegmentedControlTypeText && self.segmentWidthStyle == DZSegmentedControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == DZSegmentedControlTypeText && self.segmentWidthStyle == DZSegmentedControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == DZSegmentedControlTypeTextImages && self.segmentWidthStyle == DZSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == DZSegmentedControlTypeTextImages && DZSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        } else if (self.type == DZSegmentedControlTypeTextImages || self.type == DZSegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == DZSegmentedControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == DZSegmentedControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == DZSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
