...
- (void)dealloc
{
    [[UIApplication sharedApplication].delegate.window removeObserver:self forKeyPath:@"frame"];
}
...
- (void)saveImage
{
    ...
    [[UIApplication sharedApplication].delegate.window addSubview:indicator];
...
- (void)showFirstImage
{
    ...
    tempView.frame = rect;
    if (rect.origin.y < 0) {
        tempView.frame = CGRectMake(0, 0, screenW, screenH);
    }
