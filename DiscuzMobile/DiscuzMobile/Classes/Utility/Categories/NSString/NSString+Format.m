...
+(NSMutableAttributedString *)yy_attributedWithLeftTittle:(NSString *)title rightimage:(NSString *)imageName textColor:(UIColor*)textColor FontSize:(CGFloat)FontSize{
    ...
    NSMutableAttributedString *attchText2 = [NSMutableAttributedString attachmentStringWithContent:imgView2 contentMode:UIViewContentModeScaleAspectFit attachmentSize:imgView2.frame.size alignToFont:[UIFont systemFontOfSize:FontSize] alignment:YYTextVerticalAlignmentCenter];
...
+(NSMutableAttributedString *)attributeTextWithNormalString:(NSString *)normalString normalColor:(UIColor*)normalColor normalSize:(CGFloat)normalSize LineSpaceing:(int)lineSpacing tapString:(NSString *)tapString tapTextColor:(UIColor*)tapTextColor tapActionBlock:(void(^)(void))tapActionBlock{
    ...
    attributedText.font = [UIFont systemFontOfSize:normalSize];
    if (tapString.length) {
        [attributedText setTextHighlightRange:NSMakeRange(normalString.length+1, tapString.length) color:tapTextColor backgroundColor:[UIColor clearColor] tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
            if (tapActionBlock) {
                tapActionBlock();
            }
        }];
 ...
 */
+(NSMutableAttributedString *)yy_attributeDelLineWithString:(NSString *)textString textColor:(UIColor*)textColor FontSize:(CGFloat)FontSize LineSpaceing:(int)lineSpacing delString:(NSString *)delString delColor:(UIColor *)delColor delFontSize:(CGFloat)delFontSize{
    ...
    [text setTextStrikethrough:decoration range:NSMakeRange(textString.length + 1, delString.length)];
    return text;
}
